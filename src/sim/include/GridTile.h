/*                                              -*- mode:C++ -*-
  GridTile.h Tile for simulation
...
/**
  \file GridTile.h Tile for simulation
...
namespace MFM {
  ...
  template class GridTile; // FORWARD

  struct GridTileConfig : public TileConfig {
    typedef GridTile TILE_TYPE;
    enum { ENDPOINTS_PER_TILE = 8 };
  };

  template<class EC>
  class GridTile : public Atom< EC, GridTileConfig >
  {
    typedef GridTileConfig AC;
  ...
  protected:
    ...
    /* We really don't want to allow the public to change the type of a
       GridTile, since the type doesn't mean much without the atomic
       header as well */
  ...
  public:
    ...
    explicit GridTile(u32 type = ATOM_EMPTY_TYPE, u32 z1 = 0, u32 z2 = 0, u32 stateBits = 0)
    {
